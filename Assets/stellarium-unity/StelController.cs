...
public class StelController : MonoBehaviour
{
    ...
    private IEnumerator UpdateStelJson()
    {
        ...
        if (connectToStellarium && spoutMode) // Esp time updates are not useful while in skybox mode!
        {
            ...
            JSONObject json2 = new JSONObject(www.downloadHandler.text); // this should be a rather short JSON with only the changed actions/properties.
    ...
    private IEnumerator QueryObjectInfo(string objectName)
    {
        ...
        else
        {
            ...
            if (responseOK)
            {
                ...
                jsonObjInfo = new JSONObject(www.downloadHandler.text);
                if (objectName == "Sun")
                    //jsonSunInfo = new JSONObject(www.text);
                    jsonSunInfo = new JSONObject(www.downloadHandler.text);
                else if (objectName == "Moon")
                    //jsonMoonInfo = new JSONObject(www.text);
                    jsonMoonInfo = new JSONObject(www.downloadHandler.text);
                else if (objectName == "Venus")
                    //jsonVenusInfo = new JSONObject(www.text);
                    jsonVenusInfo = new JSONObject(www.downloadHandler.text);
            }
            else
            {
                //Debug.LogWarning("queryObjectInfo error. Headers:" + www.responseHeaders);
                Debug.LogWarning("queryObjectInfo error. Headers:" + www.GetResponseHeaders());
                //if (www.responseHeaders["STATUS"] != null)
                if (www.GetResponseHeaders()["STATUS"] != null)
                {
                    //Debug.LogWarning("queryObjectInfo(" + objectName + "): Problem with WWW answer: " + www.responseHeaders["STATUS"] + "; wait 2.5s before retrying.");
                    Debug.LogWarning("queryObjectInfo(" + objectName + "): Problem with WWW answer: " + www.GetResponseHeaders()["STATUS"] + "; wait 2.5s before retrying.");
                }
