...
using UnityEngine.Networking; // Rui Loureiro
...
public class StelSkybox : MonoBehaviour {
    ...
    private IEnumerator DoGetImages(string directory) {
        ...
        foreach(string filename in filenames) {
            ...
            sides[filename] = CropTexture(texture,new Rect((texture.width*.5f)-(texture.height*.5f),0f,texture.height,texture.height));
